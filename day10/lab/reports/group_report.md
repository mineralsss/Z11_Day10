...
Nhóm dùng raw export mẫu `data/raw/policy_export_dirty.csv`, chạy pipeline chuẩn qua `conda run -n test python etl_pipeline.py run`, sau đó kiểm tra manifest, quarantine và eval. Pipeline đọc raw, clean theo allowlist/versioning, quarantine các record lỗi, chạy expectation suite, upsert vào Chroma, rồi ghi manifest và freshness check ở boundary publish. Run gần nhất có `run_id=2026-04-15T08-54Z` trong log và các artifact đi kèm nằm trong `artifacts/manifests/`, và `artifacts/logs/`.

**Lệnh chạy một dòng (copy từ README thực tế của nhóm):**

`conda run -n test python etl_pipeline.py run`
...
|-----------------------------------|------------------|-----------------------------|-------------------------------|
| Embed idempotent / prune stale ids | collection count = 10 sau run đầu | collection count = 10 sau rerun thứ hai, không phình tài nguyên | `artifacts/logs/run_2026-04-15T08-15Z.log`, `artifacts/logs/run_2026-04-15T08-32Z.log`, kiểm tra collection count = 10 sau rerun |
...
SLA freshness của nhóm là 24 giờ, đo tại boundary `publish` theo `latest_exported_at` trong manifest. `PASS` nghĩa là snapshot mới hơn hoặc bằng SLA; `WARN` dùng cho trường hợp thiếu timestamp rõ ràng; `FAIL` là khi dữ liệu vượt SLA, như manifest mẫu hiện tại với `latest_exported_at=2026-04-10T08:00:00+00:00` nên báo `freshness_sla_exceeded`. Nhóm ghi rõ điều này trong runbook để phân biệt lỗi pipeline với dữ liệu snapshot cũ.
...
- Freshness vẫn FAIL trên snapshot mẫu vì `latest_exported_at` cũ hơn SLA 24h; đây là rủi ro dữ liệu, không phải lỗi pipeline.
...
- Nhóm chưa tích hợp Great Expectations / pydantic thật; hiện dùng expectation custom nên đây là chỗ có thể nâng cấp sau.
